...
[![x402](https://img.shields.io/badge/payments-x402%20%C2%B7%20USDC%20on%20Base-0052ff.svg)](https://x402.org)
...
PAY_TO_ADDRESS=0xYourMerchantAddress npm run dev
```
...
1. Client calls a paid route with no payment → server answers **`402 Payment Required`** with a JSON `accepts[]` describing exactly what to pay (amount, USDC contract, recipient, network).
2. Client signs an EIP-3009 USDC authorization for that amount and retries with the **`X-PAYMENT`** header.
3. The facilitator (`https://x402.org/facilitator` on testnet) **verifies and settles** the payment on Base.
...
- Mainnet: `NETWORK=base` + a production `FACILITATOR_URL`.
